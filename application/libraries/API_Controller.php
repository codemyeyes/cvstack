...
class API_Controller extends CI_Controller
{
    ...
    public function _allow_methods(array $methods)
    {
        ...
        if (in_array(strtolower($REQUEST_METHOD), $this->allowed_http_methods))
        {
            ...
            } else
            {
                ...
                $this->_response(['code' => 404, 'msg' => 'Unknown method'], self::HTTP_METHOD_NOT_ALLOWED);
            }
        } else {
            $this->_response(['code' => 404, 'msg' => 'Unknown method'], self::HTTP_METHOD_NOT_ALLOWED);
        }
    ...
    public function _limit_method(array $data)
    {
        // check limit number
        if (!isset($data[0])) {
            $this->_response(['code' => FALSE, 'msg' => 'Limit Number Required'], self::HTTP_BAD_REQUEST);
        }

        // check limit type
        if (!isset($data[1])) {
            $this->_response(['code' => FALSE, 'msg' => 'Limit Type Required'], self::HTTP_BAD_REQUEST);
        }
        
        if (!isset($this->db)) {
            $this->_response(['code' => FALSE, 'msg' => 'Load CodeIgniter Database Library'], self::HTTP_BAD_REQUEST);
        }

        // check limit database table exists
        if (!$this->db->table_exists($this->API_LIMIT_TABLE_NAME)) {
            $this->_response(['code' => FALSE, 'msg' => 'Create API Limit Database Table'], self::HTTP_BAD_REQUEST);
        }
        ...
        if ($limit_type == 'ip')
        {
            ...
            if ($this->db->affected_rows() >= $limit_num)
            {
                ...
                if (isset($limit_time) AND !empty($limit_time))
                {
                    ...
                    if (is_numeric($limit_time))
                    {
                        ...
                        if ($this->db->affected_rows() >= $limit_num)
                        {
                            $this->_response(['code' => FALSE, 'msg' => 'This IP Address has reached the time limit for this method'], self::HTTP_REQUEST_TIMEOUT);
                        } else
                        {
                            // insert limit data
                    ...
                    $this->_response(['code' => FALSE, 'msg' => 'This IP Address has reached limit for this method'], self::HTTP_REQUEST_TIMEOUT);
                }
            ...
            $this->_response(['code' => FALSE, 'msg' => 'Limit Type Invalid'], self::HTTP_BAD_REQUEST);
        }
    }
    ...
    private function _api_key(array $key)
    {
            ...
            if (isset($is_header['code']) === TRUE)
            {
                ...
                if ($api_key != "table")
                {
                    if ($HEADER_VALUE != $api_key) {
                        $this->_response(['code' => FALSE, 'msg' => 'API Key Invalid'], self::HTTP_UNAUTHORIZED);
                    }
                ...
                } else
                {
                    ...
                    if (!$this->db->table_exists($this->API_KEYS_TABLE_NAME)) {
                        $this->_response(['code' => FALSE, 'msg' => 'Create API Key Database Table'], self::HTTP_BAD_REQUEST);
                    }
                    ...
                    {
                        $this->_response(['code' => FALSE, 'msg' => 'API Key Invalid'], self::HTTP_NOT_FOUND);
                    } 
                }

            } else
            {
                $this->_response(['code' => FALSE, 'msg' => 'Set API Key in Request Header'], self::HTTP_NOT_FOUND);
            }
        } else if (strtolower($api_key_type) == 'get') // // api key type `get`
        {
            ...
            {
                $this->_response(['code' => FALSE, 'msg' => 'API Key GET Parameter Required'], self::HTTP_NOT_FOUND);
            }
        } else if (strtolower($api_key_type) == 'post') // // api key type `post`
        {
            ...
            {
                $this->_response(['code' => FALSE, 'msg' => 'API Key POST Parameter Required'], self::HTTP_NOT_FOUND);
            }

        } else {
            $this->_response(['code' => FALSE, 'msg' => 'API Key Parameter Required'], self::HTTP_NOT_FOUND);
        }
    }
    ...
    private function _isAuthorized()
    {
        ...
        if (isset($result['status']) AND $result['status'] === true)
        {
            ...
            $this->_response(['code' => FALSE, 'msg' => $result['message']], self::HTTP_UNAUTHORIZED);
        }
    }

    /**
     * Check Request Header Exists
     * @return ['code' => true, 'value' => value ]
     */
    ...
    private function _response($data = NULL, $http_code = NULL)
    {
        ...
        if (!is_array($this->return_other_data)) {
            print_r(json_encode(['code' => false, 'msg' => 'Invalid data format']));
        } else {
